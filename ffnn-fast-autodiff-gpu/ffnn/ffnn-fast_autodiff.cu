...
void initialize_weights(Matrix* m1, Matrix* m2, float lower_bound,
    float upper_bound)
{
    random_device rd;
    mt19937 gen(rd());
...
int main()
{
    ...
    MatrixColProduct* predictor = new MatrixColProduct(
        w2, new ColLeakyReLU(new MatrixColProduct(w1, featureInput)));
