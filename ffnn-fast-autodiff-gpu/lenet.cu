...
int main() {
    ...
    int c3TripleIndex[6][3] = {{0,1,2}, {1,2,3}, {2,3,4}, {3,4,5}, {4,5,0},{5,0,1}};
    for(int i=0;i<6;i++) {
        int r = c3TripleIndex[i][0];
        int s = c3TripleIndex[i][1];
        int t = c3TripleIndex[i][2];
        c3Layer[i] = new Add(new Convolution(s2Layer[r], c3LayerWeights[i], 0, 1, 0, 1),
    ...
    int c3QuadIndex[9][4] = {{0,1,2,3}, {1,2,3,4}, {2,3,4,5}, {3,4,5,0}, {4,5,0,1}, {5,0,1,2},
   ...
   for(int i=6;i<15;i++){
        ...
        c3Layer[i] = new Add(new Convolution(s2Layer[r], c3LayerWeights[i], 0, 1, 0, 1),
                            new Add(new Convolution(s2Layer[s], c3LayerWeights[i], 0, 1, 0, 1),
                                    new Add(new Convolution(s2Layer[t], c3LayerWeights[i], 0, 1, 0, 1),
                                            new Convolution(s2Layer[u], c3LayerWeights[i], 0, 1, 0, 1))));
    } 
    // the last channel is a convolution on all 6 prior channels.
    c3Layer[15] = new Add(new Convolution(s2Layer[0], c3LayerWeights[15], 0, 1, 0, 1),
        new Add(new Convolution(s2Layer[1], c3LayerWeights[15], 0, 1, 0, 1),
            new Add(new Convolution(s2Layer[2], c3LayerWeights[15], 0, 1, 0, 1),
                new Add(new Convolution(s2Layer[3], c3LayerWeights[15], 0, 1, 0, 1),
                    new Add(new Convolution(s2Layer[4], c3LayerWeights[15], 0, 1, 0, 1),
    ...
    for_each(begin(c1LayerWeights), end(c1LayerWeights), [](auto m) { initialize_weights(m, -0.05, 0.05);});
    for_each(begin(c3LayerWeights), end(c3LayerWeights), [](auto m) { initialize_weights(m, -0.05, 0.05);});
